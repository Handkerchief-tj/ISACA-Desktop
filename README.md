...
也可以直接进入同一流程，进行网表解析、层次展平、参数求值、连接与模型诊断，随后用于 SLiCAP 
的拉普拉斯传递函数、极零点、MNA 矩阵、噪声和频率响应分析。
...
在每个子区间中，算法依据支路和回路的局部主导关系执行图化简操作，并用
...
- **双输入方式**：支持从官方原理图导出网表，也支持用户自己打开和编辑 `.cir` 网表。
...
| 方式 | 需要自行安装环境 |
| --- | --- |
| 方法一：克隆源码 | 需要 Git 和 Conda |
| 方法二：安装包 | 不需要 Python/Conda |
