...
本仓库是完整的单仓库版本。桌面界面、SLiCAP 适配层和 `sfg_prototype` 算法源码
均位于 `src/`，不需要另外克隆旧网页仓库或第二个算法仓库。
...
┌──────────────────────────────────────────────────────┐
│ 电路输入                                             │
│ SLiCAP 官方原理图 (.slicap_sch) / 手写网表 (.cir)   │
└──────────────────────────┬───────────────────────────┘
                           │
                           v
┌──────────────────────────────────────────────────────┐
│ SLiCAP 官方网表导出与 ISACA 严格检查                 │
│ 连接、模型、参数、source、detector                   │
└──────────────────────────┬───────────────────────────┘
                           │
             ┌─────────────┴─────────────┐
             v                           v
┌─────────────────────────┐   ┌────────────────────────┐
│ SLiCAP 数值与符号分析   │   │ SFG 分频段符号化简    │
│ H(s)、PZ、MNA、Bode、噪声│   │ 根聚类、子区间、局部根│
└────────────┬────────────┘   └────────────┬───────────┘
             └─────────────┬───────────────┘
                           v
┌──────────────────────────────────────────────────────┐
│ 桌面结果页                                           │
│ 公式、矩阵、极零点、频率图、逐频段符号表达式        │
└──────────────────────────────────────────────────────┘
```

长时间计算通过独立 worker 进程运行，可以取消，不会阻塞原理图界面。整个第一版为
本地桌面流程，不需要启动 FastAPI、Gradio 或浏览器服务。
