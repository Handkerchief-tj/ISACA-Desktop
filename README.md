...
ISACA 采用本地桌面分层架构。这里的“前端”是由 PySide6/Qt 构建的原生 Windows
界面，而不是运行在浏览器中的 Web 前端；原理图、网表、计算任务和结果文件均保存在
用户本机。

### 桌面 UI 与交互层

主界面使用 **PySide6** 构建，并沿用 SLiCAP Structured Electronic Design
Environment 的 Qt 原生视觉风格。左侧管理项目、原理图和网表，中央显示官方原理图
画布或网表编辑器，右侧设置 source、detector、参数、分析类型和误差范围，底部显示
任务进度与日志。

分析结果使用 **QWebEngineView** 承载本地 HTML 页面，通过随程序分发的 **KaTeX**
渲染公式；波特图等数值图形由 **Matplotlib** 生成，信号流图布局由 **Graphviz**
完成。所有公式、字体和脚本均从本地资源加载，不依赖在线 CDN。

### SLiCAP 电路核心与适配层

原理图部分直接复用 **SLiCAP 5.2.1** 官方编辑器及符号库，因此器件放置、引脚、
连线、junction、元件属性、`.param`、source/detector 和 `.slicap_sch` 保存行为与
官方工具保持一致。ISACA 不再自行实现一套浏览器画布。
...
可读符号传递函数、局部零极点表达式、数值频率、主导参数和误差记录。这样既保留
SLiCAP 数值结果作为基准，也能给出更适合分析电路工作机理的分频段解析结果。
...
程序，并私有携带 Python、Qt、SLiCAP、SFG 代码和 Graphviz。源码开发与安装发行共用
同一套分析入口，避免桌面开发版和发布版采用不同算法实现。

## 核心功能

- **官方原理图编辑器**：使用 SLiCAP 5.2.1 的器件、引脚、连线、junction、参数、
  source/detector 和 `.slicap_sch` 保存格式。
- **双输入方式**：支持从官方原理图导出网表，也支持打开、编辑和验证 `.cir` 网表。
- **严格参数检查**：缺失参数不会静默补成 `1`；分析前检查器件模型、连接、引用和
  source/detector。
- **常规电路分析**：展示拉普拉斯传递函数、低频小信号增益、极点与零点、MNA
  矩阵、Bode 图和噪声结果。
- **SFG 符号化简**：提供根聚类、频率子区间、误差受控图操作、局部符号根和逐频段
  传递函数。
...
2. **输入电路**：使用中央官方画布创建/打开 `.slicap_sch`，也可以直接打开 `.cir`。
3. **设置分析端口**：填写器件参数和 `.param`，确认 source、detector；需要环路分析时
   设置 lgref。
