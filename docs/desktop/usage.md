...
左侧“新建当前项目的网表”会在当前项目的 `cir` 目录创建一个 `.cir` 模板；“打开并编辑网表”可载入
已有文件，例如 `examples/desktop/rc_lowpass.cir` 或 `demo_2_numeric.cir`。
打开后，中央“网表编辑器（可直接修改）”标签页就是当前 `.cir` 文件的编辑位置；顶部显示当前文件路径，
下方带行号的文本区域可以直接修改元件、参数、`.source` 和 `.detector`。编辑器支持基础语法高亮、
保存和未保存修改提示。右侧输入模式选择“分析当前网表”，点“验证网表”查看参数来源与缺失项。
